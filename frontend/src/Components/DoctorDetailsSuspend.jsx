...
const DoctorDetailsSuspend = () => {
    ...
    useEffect(() => {
        let doctor = JSON.parse(sessionStorage.getItem("suspendDoctor"));
    ...
    const verifyDoctor = () => {
        swal({
            title: "Are you Confirm to Suspend Doctor?",
            ...
            .then((willCreate) => {
                if (willCreate) {
                    axios.get(`${IP_ADDRS}/api/admin/suspend_doctor/${doctor.doctorId}`, { headers: { "Authorization": `Bearer ${admin.jwt}` } })
                        .then(res => {
                            console.log(res.data);
                            sessionStorage.removeItem("suspendDoctor");
                            let list = JSON.parse(sessionStorage.getItem("activeDoctorList"));
                            let newList = list.filter(d => d.doctorId != doctor.doctorId);
                            console.log(newList);
                            if (newList)
                                sessionStorage.setItem("activeDoctorList", JSON.stringify(newList));
                            swal("Doctor Suspended", "success");
                            navigate(`/doctor_list_for_suspend`)
                        })
    ...
    return (
        <div className="container-fluid" style={{ marginBottom: "50px" }}>
            ...
            <div>
                <button className="btn btn-success" onClick={verifyDoctor}>Suspend </button>
            </div>
        </div>
    );

}
export default DoctorDetailsSuspend;